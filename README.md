...
Statuses (attempted, confident, revisit), notes, and practice sessions stay in this browser. Continue practicing walks the review queue. Random hard, timed sets, topic drills, and mixed mock interviews live at `#practice`.
...
`npm test` validates the catalog schema (unique ids, required fields) and runs Playwright against every hash route, search, solution reveal, note persistence, and figure mount.
