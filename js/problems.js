window.PROBLEMS = [
  {
    ...
    solution: String.raw`
      ...
      \]
      <p>so \(\tfrac14 E_\emptyset=\tfrac52\) and \(E_\emptyset=10\). Thus \(E[T_{\mathrm{HTH}}]=10\).</p>
      ...
      \]
      <p>so \(\tfrac18 E_\emptyset=\tfrac74\) and \(E_\emptyset=14\). Thus \(E[T_{\mathrm{HHH}}]=14\).</p>
  ...
  },
  {
    ...
    solution: String.raw`
      ...
      \max_i S_i \le \tfrac12.
      \]
      <p>This fails if and only if all \(n\) points lie in some open semicircle.</p>
      ...
      \]
      <p>Check the small cases: \(n=1,2\) give probability \(1\), and two points never cover. For \(n=3\) the probability of covering is \(1-3/4=1/4\), which matches “the triangle contains the center.”</p>
      ...
      \]
      <p>with \(\mathbb{P}(N>0)=1\). Therefore</p>
      \[
      E[N] = \sum_{n=0}^\infty \mathbb{P}(N>n) = 1 + \sum_{n=1}^\infty n\,\Bigl(\tfrac12\Bigr)^{n-1}.
      \]
      <p>The generating function \(\sum_{n\ge 1} n x^{n-1}=1/(1-x)^2\) at \(x=1/2\) equals \(4\), so \(E[N]=5\).</p>
  ...
  },
  {
    ...
    solution: String.raw`
      ...
      \]
      <p>and there can be at most one cycle longer than \(n/2\). Therefore</p>
      \[
      \mathbb{P}(\text{longest cycle}>50) = \sum_{k=51}^{100}\frac1k = H_{100}-H_{50}.
      \]
      <p>The harmonic difference is</p>
      \[
      H_{100}-H_{50} = \ln 2 + O\!\left(\frac1{50}\right) \approx 0.688172,
      \]
      <p>so the success probability is</p>
      \[
      1-(H_{100}-H_{50}) \approx 0.311828 > 30\%.
      \]
      <p>As \(n\to\infty\) with a \(50\%\) opening budget the probability tends to \(1-\ln 2\approx 30.7\%\). For finite even \(n\) it is slightly larger.</p>
  ...
  },
  {
    ...
    solution: String.raw`
      ...
      \]
      <p>The count is exactly \(\mathrm{Bin}(m,\alpha)\). Five percent of a large screen is not a rare event; it is the design.</p>
      ...
      \]
      <p>Union bound gives \(\mathrm{FWER}\le 0.05\) with no independence assumption. Under independence the sharper Šidák threshold \(1-(1-\alpha)^{1/m}\) is almost the same number here.</p>
      ...
      \]
      <p>and reject \(p_{(1)},\dots,p_{(k^\star)}\) (or reject nothing if the set is empty). This controls the expected fraction of false discoveries among the rejected set.</p>
